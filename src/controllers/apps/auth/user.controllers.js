...
const registerUser = asyncHandler(async (req, res) => {
  const { email, username, password, role = UserRolesEnum.USER } = req.body;

  const existedUser = await userQueries.findOne({
    $or: [{ username }, { email }],
  });
  ...
  const createdUser = await userQueries.findById(user._id, "-password -refreshToken -emailVerificationToken -emailVerificationExpiry")
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  const loggedInUser = await userQueries.findById(user._id, "-password -refreshToken -emailVerificationToken -emailVerificationExpiry")
...
const logoutUser = asyncHandler(async (req, res) => {
  await userQueries.findByIdAndUpdate(
    req.user._id,
    {
      $set: {
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const user = await userQueries.findById(decodedToken?._id);
    if (!user) {
...
const handleSocialLogin = asyncHandler(async (req, res) => {
  const user = await userQueries.findById(req.user?._id);
